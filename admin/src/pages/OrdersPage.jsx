...
import { ShoppingBag, Eye, RefreshCw, MessageCircle, Copy, Check, ExternalLink, CheckCircle } from 'lucide-react';
...
export function OrdersPage() {
  ...
  const generateWhatsAppText = (order) => {
    if (!order) return '';
    let text = `🛒 *BAQQALA GROCERY ORDER #*${order.order_number}\n`;
    text += `👤 *Customer:* ${order.customer_name_snapshot || order.customer_name || 'Customer'}\n`;
    text += `📱 *Phone:* ${order.customer_phone_snapshot || order.customer_phone || ''}\n`;
    text += `📍 *Delivery:* ${order.customer_villa ? `Villa ${order.customer_villa}, ` : ''}${order.customer_address || order.delivery_address || ''}\n\n`;
    text += `🧺 *Items:*\n`;
    order.items?.forEach(i => {
      text += `• ${i.product_name} × ${i.quantity} — ₹${parseFloat(i.total).toFixed(2)}\n`;
    });
    text += `\n💰 *Total:* ₹${parseFloat(order.total_amount || order.total || 0).toFixed(2)} (${order.payment_method || 'COD'})\n`;
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs text-slate-700">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                <tr>
                  <td colSpan="7" className="p-8 text-center text-slate-400">Loading customer orders...</td>
                </tr>
              ) : orders.length === 0 ? (
                <tr>
                  <td colSpan="7" className="p-8 text-center text-slate-400">No customer orders found.</td>
                </tr>
              ) : (
                orders.map((o) => (
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-3xl p-6 w-full max-w-lg space-y-4 max-h-[90vh] overflow-y-auto shadow-2xl text-slate-900">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <div>
                <span className="text-[10px] uppercase font-extrabold bg-emerald-100 text-emerald-800 border border-emerald-200 px-2 py-0.5 rounded-full">
                ...
                {activeOrder.items?.map((item) => (
                  <div key={item.id} className="p-3 bg-white flex items-center justify-between text-xs">
                    <div>
                      ...
                      <span className="text-slate-500">{item.quantity} &times; ₹{parseFloat(item.unit_price).toFixed(2)}</span>
                    </div>
                    <span className="font-mono font-bold text-emerald-700">₹{parseFloat(item.total).toFixed(2)}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="pt-3 border-t border-slate-100 flex items-center justify-between font-mono font-black text-slate-900 text-base">
              <span>TOTAL DUE:</span>
              <span className="text-xl text-emerald-700">₹{parseFloat(activeOrder.total_amount || activeOrder.total || 0).toFixed(2)}</span>
            </div>
